...
const UserResponsesDetails = ({ responsesDetails }: IProps) => {
    const router = useRouter();
    const { user, isLoading } = useContext(AuthContext);

    if (isLoading) {
        return (
        <div className="w-full min-h-screen bg-slate-950 px-4 md:px-10 py-8">
            <LoadingPage />
        </div>
        );
    }
    ...
    return (
        ...
        >
            ...
            <Link
                ...
                className="text-cyan-400 hover:text-white hover:bg-cyan-800/20 rounded px-2 py-1 transition"
            >
                <ArrowLeftIcon className="h-5 w-5 inline-block mr-1" />
                Back to Dashboard
            </Link>
            <div>
                <h1 className="text-3xl font-bold text-white">Completed Forms</h1>
                <p className="text-slate-400 text-sm">Manage and view your submitted forms.</p>
            </div>
            </div>

            <div className="rounded-xl bg-gradient-to-r from-cyan-800/30 via-blue-900/30 to-cyan-800/30 border border-cyan-700 shadow-lg flex items-center p-6 md:max-w-md backdrop-blur-md">
            <div className="flex-shrink-0 bg-slate-950 rounded-full p-3 border border-cyan-700">
            ...
            <div className="overflow-x-auto">
                ...
                <tbody className="bg-gradient-to-br from-slate-950/50 via-blue-950/30 to-cyan-950/20 divide-y divide-slate-800">
                    {responsesDetails.length > 0 ? (
                    responsesDetails.map((r) => (
                        <tr key={r.id} className="hover:bg-cyan-800/10 transition">
                        <td className="px-6 py-4 whitespace-nowrap font-semibold text-white">{r.title}</td>
